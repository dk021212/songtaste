...
import com.stone.songtaste.black.indicatorFragment.ui.IndicatorFragmentActivity.TabInfo;
...
public class TitleIndicator extends LinearLayout implements
	...
	private static final int FOOTER_COLOR = 0xFFFC445;
	...
	public TitleIndicator(Context context) {
		super(context);
		this.initDraw(FOOTER_LINE_HEIGHT, FOOTER_COLOR);
	}
	...
	public TitleIndicator(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.BlackTitleIndicator);
		// Retrieve the colors to be used for this view and apply them
		int footerColor = a.getColor(
				R.styleable.BlackTitleIndicator_footerColor, FOOTER_COLOR);
		mTextColor = a
				.getColorStateList(R.styleable.BlackTitleIndicator_textColor);
		mTextSizeNormal = a.getDimension(
				R.styleable.BlackTitleIndicator_textSizeNormal, 0);
		mTextSizeSelected = a.getDimension(
				R.styleable.BlackTitleIndicator_textSizeSelected,
				mTextSizeNormal);
		mFooterLineHeight = a.getDimension(
				R.styleable.BlackTitleIndicator_footerLineHeight,
				FOOTER_LINE_HEIGHT);
		mFooterTriangleHeight = a.getDimension(
				R.styleable.BlackTitleIndicator_footerTriangleHeight,
				FOOTER_TRIANGLE_HEIGHT);

		this.initDraw(mFooterLineHeight, footerColor);
		a.recycle();
	}

	/*
	 * @这个是核心函数，选项卡使用canvas画出来的。所有的 invalidate方法均会触发onDraw
	 * 大意是这样：当页面滚动的时候，会有一个滚动距离，然后onDraw被触发后， 就会在新位置重新画上滚动条（其实就是画线）
	 */
	@Override
	protected void onDraw(Canvas canvas) {
		...
		path.rewind();
		float offset = 0;
		float left_x = mSelectedTab * mPreItemWidth + offset + scroll_x;
		float right_x = (mSelectedTab + 1) * mPreItemWidth - offset + scroll_x;
	...
	public void updateChildTips(int position, boolean showTips) {
	...
	protected void add(int index, String label, int icon, boolean hasTips) {
		...
		if (index < 2) {
			tabIndicator = mInflater.inflate(
					R.layout.black_title_flow_indicator, this, false);
		} else {
			tabIndicator = mInflater.inflate(
					R.layout.black_title_flow_indicator_v2, this, false);
		}
	...
	public void setDisplayPage(int index) {
		mSelectedTab = index;
	}
